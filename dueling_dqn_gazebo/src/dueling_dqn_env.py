...
class Env():
    ...
    def setReward(self, done, action):
        ...
        Phần thưởng càng cao khi robot càng gần mục tiêu.

        Args:
            done (bool): Trạng thái kết thúc (True nếu robot va chạm hoặc đạt mục tiêu).
            action (int): Hành động hiện tại của robot.

        Returns:
            tuple: reward (float), goal_counters (int).
        """
        # Tính khoảng cách giữa robot và mục tiêu
        distance_to_goal = ((self.position.x - self.goal_x)**2 + (self.position.y - self.goal_y)**2)**0.5

        # Phần thưởng mặc định là ngược với khoảng cách đến mục tiêu
        reward = 1.0 / (distance_to_goal + 1e-6)  # Cộng 1e-6 để tránh chia cho số 0

        # Nếu robot va chạm (rơi vào hồ băng)
        if done:
            rospy.loginfo("*****************")
            rospy.loginfo("* COLLISION !!! *")
            rospy.loginfo("*****************")
            reward = -1.0  # Phạt khi va chạm
            self.pub_cmd_vel.publish(Twist())  # Dừng robot

            # Kiểm tra nếu robot đã đạt đến mục tiêu
            if self.position.x == self.goal_x and self.position.y == self.goal_y:
                rospy.loginfo("Goal reached!")
                reward = 10.0  # Phần thưởng lớn khi robot đến mục tiêu
